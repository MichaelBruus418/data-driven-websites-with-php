  ...
  <div class="container py-3">
    <header class="mb-auto">
      ...
      <div class="pricing-header p-3 pb-md-4 mx-auto text-center">
        ...
        <p class="fs-5 text-muted">Please see our available services and pricing below, then <a
            href="contact.php">Contact Us</a> to book an appointment.</p>
      </div>
    </header>
    ...
    <main>
      <div class="row row-cols-1 row-cols-md-3 mb-3 text-center">
        <!-- Wash Card -->
        <?php
        $query = $db->prepare("select price from services where name = :name");
        $success = $query->execute(['name' => "wash"]);
        ['price' => $price] = $query->fetch(PDO::FETCH_ASSOC);
        ?>
        <div class="col">
        ...
        <?php
        $query = $db->prepare("select price from services where name = :name");
        $success = $query->execute(['name' => "trim"]);
        ['price' => $price] = $query->fetch(PDO::FETCH_ASSOC);
        ?>
        <div class="col">
          <div class="card mb-4 rounded-3 shadow-sm">
        ...
        <?php
        $query = $db->prepare("select price from services where name = :name");
        $success = $query->execute(['name' => "full grooming"]);
        ['price' => $price] = $query->fetch(PDO::FETCH_ASSOC);
        ?>
        <div class="col">
